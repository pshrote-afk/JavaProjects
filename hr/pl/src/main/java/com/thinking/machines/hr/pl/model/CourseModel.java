...
public class CourseModel extends AbstractTableModel {
  ...
  private void populateDataStructure() {
    ...
    Collections.sort(
        this.plCoursesList,
        new Comparator<CourseInterface>() {
  ...
  public void add(CourseInterface course) throws BLException {
    courseManager.addCourse(
        course); // not in a try-catch cause method does not handle error, it throws it
  ...
  public void exportToPDF(File file) throws BLException {
    ...
    try {
      ...
      while (r < plCoursesList.size()) {
        ...
        {
          Image companyLogo =
              new Image(ImageDataFactory.create(getClass().getResource("/icons/logo.png")));
        ...
        pdfTable.addCell(
            new Cell()
                .add(new Paragraph(plCoursesList.get(r).getTitle()))
  ...
  public int indexOfCourse(CourseInterface course) throws BLException {
    ...
    while (iterator.hasNext()) {
      tmpCourse = iterator.next();
      if (tmpCourse.equals(
          course)) // deep comparison. CourseInterface implements Comparable which means we have
                   // written "equals()" method which compares on the basis of "course code" in its
                   // implementation
      {
        break;
      }
    ...
    if (index
        == plCoursesList
            .size()) // control will never reach here, cause if course doesnt exist then BL will
                     // indicate even before control enters this function
    {
      BLException blException = new BLException();
    ...
    while (iterator.hasNext()) {
      ...
      if (leftPartialSearch) {
        if (tmpCourse
            .getTitle()
            .toUpperCase()
            .startsWith(title.toUpperCase())) // rethink //done done
        {
          return index;
        }
      } else {
